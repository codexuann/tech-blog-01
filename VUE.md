...
#### 指令与API
- v-if
	- 条件渲染, 控制dom元素的创建和销毁, 性能开销大
	- 如果一个元素在运行时很少切换(如用户权限控制, 不是管理员永远不显示), 如果开始v-if为false, 则根本不会渲染这块内容, 节省首屏加载开销
- v-show
	- 只触发css变换, 修改css display 属性 (会触发回流和重绘), 不论true还是false, dom都存在
	- 如果一个元素频繁切换, 用v-show更合适, 因为切换省去了dom的创建销毁开销
- v-for
	- key是虚拟dom节点的唯一标识, 若没有key, 例如在列表头部插入一个数据, vue会认为列表所有节点都发生变化, 所有节点都更新一遍, 如果列表中有输入框, 输入框的位置不会变化从而状态错乱(diff没有对dom进行删除重建而是就地复用). 如果有key, diff算法得知列表的其余节点只是发生了移动, 只会对真实dom节点进行移动, 不会销毁重建,大大提升性能
	- v-if和v-for为什么不能连用
		- vue2: v-for的优先级高于v-if. 如果只想显示一两个元素, vue仍会把整个列表循环一遍在每个循环再去判断v-if, 造成巨大的性能浪费
		- vue3: v-for的优先级低于v-if, 但v-if执行是v-for的变量没有执行, 如果v-if访问v-for里的当前项, 会报错
		- 解决方案
			- Computed计算属性把需要显示的数据用filter过滤出来
			- 在外套一层`<template v-for="...">`, 利用template不渲染真实dom来区分逻辑
- v-model
	- v-model是属性绑定(v-bind)和事件更新(v-on)的语法糖
	- 在原生input组件, 属性绑定是 `:value`, 事件更新是 `@input`
	- 在自定义组件中, 属性绑定是 `:modelValue`, 内部数据变化时(如输入了新内容). 会触发一个叫 `update:modelValue`的事件
- vue语法糖
	- v-bind  `:`
	- v-on  `@`
	- v-slot `#`
...
- computed
	- 机制: 基于已有的响应式数据, 计算并返回得到一个新值
	- 有缓存, 只有当响应式数据发生变化, 才重新计算; 如果依赖没变, 访问则是返回缓存
	- 必须有返回值 `return`, 且强烈建议是纯函数
	- 场景: 列表过滤, 数据处理(如格式化)
- watch
	- 机制: 显示监听一个或多个响应式数据, 当数据变化时, 执行回调函数
	- 场景: 处理副作用, 如数据变化后的网络请求, 存localstorage, 操作dom, 执行截流防抖逻辑
	- 告知具体监听哪个数据, 可在回调函数中接收`(newVal, oldVal)`两个参数
	- `deep:true` 开启对对象的深层监听
	- `immediate:true` 初始化时就执行, 默认情况是只有数据变化了才去执行
	- 监听reactive包裹的对象时数据改变`oldValue`和`newValue`值是一样的
		- 正确的操作方法是使用`getter`函数: watch中第一个参数从传一个响应式对象`user`改成传`()=>user.name`或`()=>user.value.name
- watcheffect
	- 机制: 接收一个回调函数并且立即执行, 执行时自动收集依赖(函数里用到的所有响应式数据), 只要依赖发生变化, 回调函数就会重新执行
	- 只关心执行动作, 拿不到旧值
	- 场景: 多个状态关联的复杂副作用, 其中一个状态改变执行整个副作用
- 